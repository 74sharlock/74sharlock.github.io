...
export default function init() {
  ...
  function Dragend(container, settings) {
    ...
    this._scroll = supportTransform
      ? this._scrollWithTransform
      : this._scrollWithoutTransform;
    this._animateScroll = supportTransform
  ...
  extend(Dragend.prototype, {
        ...
        case 'up':
          if (
            ...
            coordinates.y = Math.round(
              -(
                (Math.ceil(this.pagesCount) - 1) *
                (this.pageDimentions.height + this.settings.borderBetweenPages)
              ) +
                y / 5
            );
            return coordinates;
    ...
    _onMove: function(event) {
      ...
      var parsedEvent = this._parseEvent(event),
        coordinates = this._checkOverscroll(
          parsedEvent.direction,
          -parsedEvent.distanceX,
          -parsedEvent.distanceY
        );
    ...
    _onEnd: function(event) {
      ...
      if (
        Math.abs(parsedEvent.distanceX) > this.settings.minDragDistance ||
        Math.abs(parsedEvent.distanceY) > this.settings.minDragDistance
      ) {
        this.swipe(parsedEvent.direction);
      } else if (
        Math.abs(parsedEvent.distanceX) > 0 ||
        Math.abs(parsedEvent.distanceY) > 0
      ) {
        this._scrollToPage();
      }
      ...
      switch (direction) {
        case 'up':
          if (page < this.pagesCount - 1) {
            this.scrollBorder.y =
              this.scrollBorder.y + height + borderBetweenPages;
            this.page++;
          }
          break;
        ...
        case 'left':
          if (page < this.pagesCount - 1) {
            this.scrollBorder.x =
              this.scrollBorder.x + width + borderBetweenPages;
            this.page++;
          }
          break;
    ...
    _scrollWithTransform: function(coordinates) {
      var style =
        this.settings.direction === 'horizontal'
    ...
    _animateScrollWithoutTransform: function() {
      var property =
          this.settings.direction === 'horizontal' ? 'marginLeft' : 'marginTop',
        value =
          this.settings.direction === 'horizontal'
            ? -this.scrollBorder.x
            : -this.scrollBorder.y;
    ...
    updateInstance: function(settings) {
      ...
      this.pages =
        settings.pages ||
        getElementsByClassName(this.settings.pageClass, this.pageContainer);
